from dpdt import DPDTreeClassifier, DPDTreeClassifierDepthFirst
...
def test_dpdt__depth_first_is_same_learning(n_samples, n_features, max_depth, max_nb_trees, cart_nodes_list):
    X, y = make_blobs(n_samples, centers=2, n_features=n_features, random_state=0)
    y = y.reshape(-1, 1)
    clf_dfs = DPDTreeClassifierDepthFirst(max_depth, max_nb_trees, cart_nodes_list, random_state=0)
    clf_bfs = DPDTreeClassifier(max_depth, max_nb_trees, cart_nodes_list, random_state=0)
...
def test_check_estimator(max_depth, max_nb_trees, cart_nodes_list):
    check_estimator(DPDTreeClassifierDepthFirst(max_depth, max_nb_trees, cart_nodes_list))
